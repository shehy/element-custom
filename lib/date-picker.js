...
var TYPE_VALUE_RESOLVER_MAP = {
  ...
  dates: {
    formatter: function formatter(value, format) {
      return isValidDate(value) ? value.map(function (date) {
    ...
    displayValue: function displayValue() {
      ...
      if (this.buddhistYear) {
        if (Array.isArray(this.parsedValue)) {
          parsedValue = [];
          this.parsedValue.forEach(function (dat) {
            var nd = new Date(dat.valueOf());
            nd.setFullYear(nd.getFullYear() + 543);
            parsedValue.push(nd);
          });
        } else if (this.parsedValue) {
          var nd = new Date(this.parsedValue.valueOf());
          nd.setFullYear(nd.getFullYear() + 543);
          parsedValue = new Date(nd.valueOf());
        }
      }
      var formattedValue = formatAsFormatAndType(parsedValue, this.format, this.type, this.rangeSeparator);
  ...
  computed: {
    ...
    visibleDate: function visibleDate() {
      if (this.userInputDate !== null) {
        ...
        var dat = this.value || this.defaultValue;
        var nd = new Date(dat.valueOf());
        nd.setFullYear(nd.getFullYear() + 543);
        return Object(date_util_["formatDate"])(nd, this.dateFormat);
      }
    },
    yearLabel: function yearLabel() {
      var yearTranslation = this.t('el.datepicker.year');
      var displayYear = this.buddhistYear && !this.userInputDate ? this.year + 543 : this.year;
      if (this.currentView === 'year') {
        var startYear = Math.floor(displayYear / 10) * 10;
        if (yearTranslation) {
...
/* harmony default export */ var date_rangevue_type_script_lang_js_ = ({
  ...
  computed: {
    ...
    leftLabel: function leftLabel() {
      var year = this.buddhistYear ? this.leftDate.getFullYear() + 543 : this.leftDate.getFullYear();
      return year + this.t('el.datepicker.year') + ' ' + this.t('el.datepicker.month' + (this.leftDate.getMonth() + 1));
    },
    rightLabel: function rightLabel() {
      var year = this.buddhistYear ? this.rightDate.getFullYear() + 543 : this.rightDate.getFullYear();
...
/* harmony default export */ var month_rangevue_type_script_lang_js_ = ({
  ...
  computed: {
    ...
    rightLabel: function rightLabel() {
      var year = this.buddhistYear ? this.rightDate.getFullYear() + 543 : this.leftDate.getFullYear();
      return year + ' ' + this.t('el.datepicker.year');
    },
    leftYear: function leftYear() {
      return this.leftDate.getFullYear();
